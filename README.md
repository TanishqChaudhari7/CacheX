...
worth **+45%** throughput (median of 5 runs), measured before and after. 212 tests
pass under ASan,
...
| ✅ Stage 9 | Benchmark suite, profiling, `get_into()` (+45%), 212 tests |
...
**221 measured** in memory — it stores only the data, because loading rebuilds the index.
...
| | |
| --- | --- |
| Single-threaded, read-heavy | **6.0 M ops/sec**, p50 125 ns, 78.95% hit rate |
| Sharded vs global mutex, 4 threads | **+175% to +235%** throughput, **−69% to −76%** p99 |
| Profiling-driven optimisation | **+45%** from removing a per-hit allocation |
| Memory | **221 bytes/entry** measured (2.76× an 80-byte payload) |

All figures are the **median of 5 captured runs**; the raw output is committed in
[`benchmarks/results/`](benchmarks/results) and regenerated by
`./benchmarks/run_all.sh`, which also records the machine, commit and load average
it ran under. Every number quoted anywhere in this repo can be found in one of
those files.
...
│   ├── run_all.sh              captures every benchmark into results/
│   ├── results/                committed raw output + environment record
